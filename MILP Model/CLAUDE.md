...
- NEVER 移項、改號、翻轉比較方向、合併化簡 —— ALWAYS AML 左側項 → `AddLHS(...)`、右側項 → `AddRHS(...)`，`>=` → `CreateGreatEqual`、`<=` → `CreateLessEqual`、`=` → `CreateEqual` —— Why: 轉譯必須可逐條對照 Model.md 驗證，動過手腳就驗不了
...
$OPT/ClaudeAIAssistant/CPLEX_API_REFERENCE.md - 消費端 API 對照（含 soft constraint、Experiment）
$OPT/ClaudeAIAssistant/claudemdTemplate/ - 專案內各資料夾 CLAUDE.md 模板（單一來源）
$OPT/ClaudeAIAssistant/Projects/HospitalRostering_Generator - 預設（generator + OptModel）可運作範例
$OPT/ClaudeAIAssistant/Projects/HospitalRostering_Manual - 手寫後路可運作範例
$OPT/ClaudeAIAssistant/dlls/ - DLL 唯一來源（HintPath 一律指向這裡）
</file_map>
...
2. **Foundation Coding**：讀 `Foundation Coding/CLAUDE.md` → 在 `$OPT/ClaudeAIAssistant/Projects/<Project>/` 依五資料夾結構逐條轉譯 → `dotnet build` → fix loop（≤5 次）→ `dotnet run` → 解驗證協定（四步）
...
```csharp
// AML: sum_g Produce_g * UsageRate_g <= MachineCapacity
...
- API 簽名不確定 → 查 `$OPT/ClaudeAIAssistant/CPLEX_API_REFERENCE.md`，NEVER 憑記憶寫
- 改本 domain 規範 → 跑 `/harness-eval milp-model` 回歸
</common_tasks>

<hatch>
- `AI Modeling - Claude Code/` 的 Web API pipeline（16-stage Prompts + PromptLibrary）是獨立產品，維護它時用它自己的 CLAUDE.md，不套本 domain 互動流程
...
- NEVER 改 OptimFoundation 框架本體（唯讀）—— 要擴充在專案端寫 helper；真要改框架 → Core + Cplex DLL 一起 rebuild 並更新 `$OPT/ClaudeAIAssistant/dlls/`
</fatal_implications>
